...
|`10001` (par défaut — configurable via `WIS2DOWNLOADER_UID` dans `.env`)

|Nom du groupe
|`wis2`

|Identifiant de groupe (GID)
|`988` (par défaut — configurable via `WIS2DOWNLOADER_GID` dans `.env`)
|===

Le worker Celery écrit les fichiers dans `/data` à l'intérieur du conteneur, ce qui correspond à `HOST_DATA_PATH` sur l'hôte (par défaut : `./downloads/`).

IMPORTANT: Si vous modifiez `WIS2DOWNLOADER_UID` ou `WIS2DOWNLOADER_GID` dans `.env`, reconstruisez les images avec `docker compose build` avant de redémarrer, et utilisez les valeurs correspondantes dans les commandes ci-dessous.
...
|integer
|`5001`
|Port d'écoute de l'API (port interne du conteneur)

|`WIS2DOWNLOADER_UID`
|integer
|`10001`
|Identifiant utilisateur utilisé lors de la construction des images. Doit correspondre au propriétaire du répertoire hôte pour les écritures de volume.

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|Identifiant de groupe utilisé lors de la construction des images. Doit correspondre au groupe du répertoire hôte pour les écritures de volume.

|`HOST_DATA_PATH`
|path
|`./downloads`
|Chemin hôte monté comme volume de téléchargement

|`CONTAINER_DATA_PATH`
|path
|`/data`
|Point de montage à l'intérieur des conteneurs
|===
...
|`WIS2DOWNLOADER_BASE_URL`
|string
|`http://localhost`
|URL publique de base du déploiement (IP ou nom d'hôte du serveur). Définissez cette valeur à l'adresse de votre serveur lors d'un déploiement distant.

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|URL publique de l'API du gestionnaire d'abonnements (côté navigateur)

|`WIS2DOWNLOADER_GRAFANA_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:3000`
|URL publique utilisée pour intégrer les panneaux Grafana dans la vue Tableau de bord
...
La pile exécute trois pools de workers Celery, chacun ciblant une file dédiée :

[cols="2,1,1,3"]
|===
|Service |File |Concurrence par défaut |Rôle

|`celery-workers-high-priority`
|`high_priority`
|8
|Téléchargements haute priorité

|`celery-workers-small-files`
|`small_files`
|24
|File par défaut pour les petits fichiers

|`celery-workers-large-files`
|`large_files`
|8
|Téléchargements de grands fichiers
|===

Ajustez la concurrence en modifiant le flag `--concurrency` dans la commande du service concerné dans `docker-compose.yaml` :

[source,yaml]
----
celery-workers-small-files:
  command: ["/bin/bash", "-c", "source .venv/bin/activate && task_manager_start worker --pool=threads --concurrency=32 --loglevel=info -Q small_files"]
----

Lors de la création d'un abonnement via l'API, utilisez le champ `queue` pour router les téléchargements vers un pool spécifique (`high_priority`, `small_files` ou `large_files`). La valeur par défaut est `small_files`.
...
# Service spécifique
docker-compose logs -f celery-workers-small-files
...
[source,bash]
----
docker-compose logs celery-workers-small-files | tail -50
----
