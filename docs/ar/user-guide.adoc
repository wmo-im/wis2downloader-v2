...
يُتيح درج التنقل على اليسار (اليمين في وضع RTL العربي) الوصول إلى سبع طرق عرض. كل منها قابلة للوصول أيضًا من خلال اختصار لوحة مفاتيح:
...
|kbd:[Alt+4]
|*اشتراك يدوي*
|إدخال موضوع ومجلد حفظ وفلتر وقائمة انتظار وبيانات اعتماد مباشرةً دون استعراض الكتالوج

|kbd:[Alt+5]
|*إدارة الاشتراكات*
|سرد جميع الاشتراكات النشطة وتعديل إعداداتها وإلغاء الاشتراك في الموضوعات
...
* *عرض الترخيص* — يفتح رابط ترخيص مجموعة البيانات في تبويب جديد (يظهر فقط عندما يحتوي السجل على رابط ترخيص)
* *تحديد* / *إلغاء التحديد* — يُضيف أو يُزيل موضوع MQTT لمجموعة البيانات إلى/من الشريط الجانبي للاشتراك

NOTE: إذا كانت مجموعة البيانات تحمل ترخيصًا، فإن النقر على *تحديد* يُظهر مربع حوار تأكيد الترخيص. يجب عليك عرض الترخيص والموافقة عليه قبل إضافة الموضوع إلى الشريط الجانبي للاشتراك.
...
|قائمة الانتظار
|قائمة انتظار ذات أولوية لمعالجة التنزيل: `high_priority` أو `small_files` (الافتراضي) أو `large_files`. استخدم `large_files` للموضوعات الضخمة أو كبيرة الحجم لتجنب تأخير التنزيلات الأصغر.
|===
...
=== بيانات الاعتماد (الاشتراك اليدوي)

تتضمن طريقة عرض *الاشتراك اليدوي* (kbd:[Alt+4]) قسمًا للمصادقة للمصادر التي تتطلب بيانات اعتماد. حدد نوع المصادقة بزر الراديو:
...
|لا شيء (افتراضي)
|بدون مصادقة — للمصادر العامة

|أساسي
|اسم المستخدم وكلمة المرور

|حامل
|رمز مميز (يُرسَل كـ `Authorization: Bearer <token>`)
|===

NOTE: تُخزَّن بيانات الاعتماد في Redis وتُرسَل مع كل طلب تنزيل للاشتراك. تُحجَب كلمة المرور والرمز المميز في ردود API.

=== إدارة الاشتراكات

تسرد طريقة عرض الاشتراكات جميع الاشتراكات النشطة مع موضوعها ومسار الحفظ وملخص الفلتر. يتوفر إجراءان على كل إدخال:

* *تعديل* — يفتح مربع حوار لتعديل مجلد الحفظ والفلتر وقائمة الانتظار وبيانات الاعتماد للاشتراك. لا يمكن تغيير الموضوع؛ للاشتراك في موضوع مختلف يجب إنشاء اشتراك جديد.
* *إلغاء الاشتراك* — يُزيل الاشتراك. إذا كان هذا آخر اشتراك للموضوع، يُغلَق اتصال MQTT أيضًا.

استخدم *إعادة تحميل الاشتراكات* لتحديث القائمة بعد أي تغييرات أُجريت عبر API.
...
|`queue`
|لا
|قائمة انتظار ذات أولوية للمعالجة: `high_priority` أو `small_files` (الافتراضي) أو `large_files`

|`credentials`
|لا
|المصادقة لمصدر البيانات (انظر <<بيانات-الاعتماد-api>>)
|===
...
حدِّث مجلد الحفظ أو الفلتر أو قائمة الانتظار أو بيانات الاعتماد لاشتراك دون إعادة إنشائه:
...
تتغير الحقول المُدرَجة فقط؛ تحتفظ الحقول المحذوفة بقيمها الحالية. يمكن تعيين `credentials` إلى `null` لإزالة المصادقة.
...
تقبل الاشتراكات التي تُنزِّل من مصادر محمية بكلمة مرور أو رمز مميز كائن `credentials` اختياريًا.
...
NOTE: تحجب واجهة API الأسرار في الردود — يُحذف `password` ويُستبدل `token` بـ `***`. تُخزَّن بيانات الاعتماد الكاملة في Redis وتُستخدَم فقط أثناء التنزيل.
