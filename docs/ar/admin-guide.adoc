...
|معرّف المستخدم (UID)
|`10001` (الافتراضي — قابل للتهيئة عبر `WIS2DOWNLOADER_UID` في `.env`)

|اسم المجموعة
|`wis2`

|معرّف المجموعة (GID)
|`988` (الافتراضي — قابل للتهيئة عبر `WIS2DOWNLOADER_GID` في `.env`)
|===

يكتب عامل Celery الملفات في `/data` داخل الحاوية، والذي يُخصَّص للمسار `HOST_DATA_PATH` على المضيف (الافتراضي: `./downloads/`).

IMPORTANT: إذا غيّرت `WIS2DOWNLOADER_UID` أو `WIS2DOWNLOADER_GID` في `.env`، أعد بناء الصور بـ `docker compose build` قبل إعادة التشغيل، واستخدم القيم المطابقة في الأوامر أدناه.
...
|integer
|`10001`
|معرّف المستخدم المستخدم عند بناء صور الحاويات. يجب أن يتطابق مع مالك مجلد المضيف لعمليات الكتابة على وحدة التخزين.

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|معرّف المجموعة المستخدم عند بناء صور الحاويات. يجب أن يتطابق مع مجموعة مجلد المضيف لعمليات الكتابة على وحدة التخزين.

|`HOST_DATA_PATH`
|path
|`./downloads`
|مسار المضيف المُثبَّت كوحدة تخزين التنزيل

|`CONTAINER_DATA_PATH`
|path
|`/data`
|نقطة التثبيت داخل الحاوية حيث تُكتب الملفات المُنزَّلة
|===
...
|string
|`http://localhost`
|عنوان URL العام الأساسي للنشر (IP الخادم أو اسم المضيف). يُشكِّل الافتراضيات للمتغيرَيْن أدناه. عيِّنه بعنوان خادمك عند النشر عن بُعد.

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|عنوان URL لـ API مدير الاشتراكات المواجه للمتصفح
...
يُشغِّل المكدّس ثلاثة مجموعات من عمال Celery، كل منها يستهدف قائمة انتظار مخصصة:

[cols="2,1,1,3"]
|===
|الخدمة |قائمة الانتظار |التزامن الافتراضي |الغرض

|`celery-workers-high-priority`
|`high_priority`
|8
|التنزيلات ذات الأولوية العالية

|`celery-workers-small-files`
|`small_files`
|24
|قائمة الانتظار الافتراضية للملفات الصغيرة

|`celery-workers-large-files`
|`large_files`
|8
|تنزيلات الملفات الكبيرة
|===

عدِّل التزامن بتغيير خيار `--concurrency` في `command` الخاصة بالخدمة المعنية في `docker-compose.yaml`:
...
عند إنشاء اشتراك عبر API، استخدم حقل `queue` لتوجيه التنزيلات إلى مجموعة محددة (`high_priority`، `small_files`، أو `large_files`). القيمة الافتراضية هي `small_files`.
