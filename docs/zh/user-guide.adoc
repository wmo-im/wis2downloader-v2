...
NOTE: 如果数据集附带许可证，点击 *选择* 将显示许可证确认对话框。必须查看并接受许可证，才能将主题添加到订阅侧边栏。
...
|队列
|下载处理的优先级队列：`high_priority`、`small_files`（默认）或 `large_files`。对于大批量或大文件主题，使用 `large_files` 以避免阻塞较小的下载。
|===
...
NOTE: 凭据存储在 Redis 中，并随订阅的每次下载请求一起发送。密码和令牌在 API 响应中会被脱敏处理。
...
|`queue`
|否
|处理优先级队列：`high_priority`、`small_files`（默认）或 `large_files`
...
NOTE: API 在响应中隐藏敏感信息——`password` 被省略，`token` 被替换为 `***`。完整凭据存储在 Redis 中，仅在下载时使用。
