...
|用户 ID（UID）
|`10001`（默认值 — 可通过 `.env` 中的 `WIS2DOWNLOADER_UID` 配置）

|组名
|`wis2`

|组 ID（GID）
|`988`（默认值 — 可通过 `.env` 中的 `WIS2DOWNLOADER_GID` 配置）
|===

Celery worker 在容器内将文件写入 `/data`，对应主机上的 `HOST_DATA_PATH`（默认：`./downloads/`）。

IMPORTANT: 如果在 `.env` 中更改了 `WIS2DOWNLOADER_UID` 或 `WIS2DOWNLOADER_GID`，请在重启前执行 `docker compose build` 重新构建镜像，并在下方命令中使用对应的值。
...
|integer
|`10001`
|构建容器镜像时使用的用户 ID。必须与卷写入所需的主机目录所有者匹配。

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|构建容器镜像时使用的组 ID。必须与卷写入所需的主机目录所属组匹配。
...
|string
|`http://localhost`
|部署的公共基础 URL（服务器 IP 或主机名）。用于驱动下方两个变量的默认值。远程部署时请设置为服务器地址。

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|浏览器访问订阅管理器 API 的 URL
...
该服务栈运行三个 Celery worker 池，每个池对应一个专用队列：
...
通过修改 `docker-compose.yaml` 中相应服务的 `command` 中的 `--concurrency` 参数来调整并发数：
...
通过 API 创建订阅时，使用 `queue` 字段将下载路由到指定的 worker 池（`high_priority`、`small_files` 或 `large_files`）。默认值为 `small_files`。
