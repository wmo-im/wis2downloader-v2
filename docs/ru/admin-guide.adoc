...
|`10001` (по умолчанию — настраивается через `WIS2DOWNLOADER_UID` в `.env`)

|Имя группы
|`wis2`

|Идентификатор группы (GID)
|`988` (по умолчанию — настраивается через `WIS2DOWNLOADER_GID` в `.env`)
|===

Воркер Celery записывает файлы в `/data` внутри контейнера, что соответствует `HOST_DATA_PATH` на хосте (по умолчанию: `./downloads/`).

IMPORTANT: Если вы изменяете `WIS2DOWNLOADER_UID` или `WIS2DOWNLOADER_GID` в `.env`, пересоберите образы командой `docker compose build` перед перезапуском и используйте соответствующие значения в командах ниже.
...
|integer
|`5001`
|Порт привязки API (внутренний порт контейнера)

|`WIS2DOWNLOADER_UID`
|integer
|`10001`
|Идентификатор пользователя, используемый при сборке образов. Должен совпадать с владельцем каталога на хосте для записи в том.

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|Идентификатор группы, используемый при сборке образов. Должен совпадать с группой каталога на хосте для записи в том.

|`HOST_DATA_PATH`
|path
|`./downloads`
|Путь на хосте, монтируемый как том загрузок

|`CONTAINER_DATA_PATH`
|path
|`/data`
|Точка монтирования внутри контейнеров
|===
...
|string
|`http://localhost`
|Публичный базовый URL развёртывания (IP-адрес или имя хоста сервера). Установите адрес вашего сервера при удалённом развёртывании.

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|Публичный URL API менеджера подписок (на стороне браузера)

|`WIS2DOWNLOADER_GRAFANA_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:3000`
|Публичный URL для встраивания панелей Grafana в вид Панели управления
...
Стек запускает три пула воркеров Celery, каждый из которых обслуживает отдельную очередь:

[cols="2,1,1,3"]
|===
|Сервис |Очередь |Параллелизм по умолчанию |Назначение

|`celery-workers-high-priority`
|`high_priority`
|8
|Высокоприоритетные загрузки

|`celery-workers-small-files`
|`small_files`
|24
|Очередь по умолчанию для небольших файлов
...
Настройте параллелизм, изменив флаг `--concurrency` в команде соответствующего сервиса в `docker-compose.yaml`:
...
При создании подписки через API используйте поле `queue` для маршрутизации загрузок в конкретный пул (`high_priority`, `small_files` или `large_files`). Значение по умолчанию: `small_files`.
