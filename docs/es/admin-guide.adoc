...
|`10001` (por defecto — configurable mediante `WIS2DOWNLOADER_UID` en `.env`)

|Nombre del grupo
|`wis2`

|Identificador de grupo (GID)
|`988` (por defecto — configurable mediante `WIS2DOWNLOADER_GID` en `.env`)
|===

El worker Celery escribe archivos en `/data` dentro del contenedor, que se mapea a `HOST_DATA_PATH` en el host (por defecto: `./downloads/`).

IMPORTANT: Si cambia `WIS2DOWNLOADER_UID` o `WIS2DOWNLOADER_GID` en `.env`, reconstruya las imágenes con `docker compose build` antes de reiniciar, y use los valores correspondientes en los comandos a continuación.
...
|integer
|`5001`
|Puerto de enlace de la API (puerto interno del contenedor)

|`WIS2DOWNLOADER_UID`
|integer
|`10001`
|ID de usuario utilizado al construir las imágenes. Debe coincidir con el propietario del directorio host para las escrituras de volumen.

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|ID de grupo utilizado al construir las imágenes. Debe coincidir con el grupo del directorio host para las escrituras de volumen.

|`HOST_DATA_PATH`
|path
|`./downloads`
|Ruta del host montada como volumen de descargas

|`CONTAINER_DATA_PATH`
|path
|`/data`
|Punto de montaje dentro de los contenedores
|===
...
|string
|`http://localhost`
|URL pública base del despliegue (IP o nombre de host del servidor). Establezca la dirección de su servidor en despliegues remotos.

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|URL pública de la API del gestor de suscripciones (vista desde el navegador)

|`WIS2DOWNLOADER_GRAFANA_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:3000`
|URL pública utilizada para incrustar paneles de Grafana en la vista Panel de control
...
La pila ejecuta tres grupos de workers Celery, cada uno dirigido a una cola dedicada:

[cols="2,1,1,3"]
|===
|Servicio |Cola |Concurrencia por defecto |Propósito

|`celery-workers-high-priority`
|`high_priority`
|8
|Descargas de alta prioridad

|`celery-workers-small-files`
|`small_files`
|24
|Cola por defecto para archivos pequeños
...
Ajuste la concurrencia editando el flag `--concurrency` en el `command` del servicio correspondiente en `docker-compose.yaml`:
...
Al crear una suscripción mediante la API, use el campo `queue` para enrutar las descargas a un grupo específico (`high_priority`, `small_files` o `large_files`). El valor predeterminado es `small_files`.
