...
El cajón de navegación a la izquierda (a la derecha en modo RTL árabe) da acceso a siete vistas. Cada una también es accesible mediante un atajo de teclado:
...
|kbd:[Alt+4]
|*Suscripción manual*
|Introduzca un tema, directorio de guardado, filtro, cola y credenciales directamente sin explorar el catálogo

|kbd:[Alt+5]
|*Gestionar suscripciones*
|Lista todas las suscripciones activas, edita su configuración y permite cancelar la suscripción a temas
...
* *Ver licencia* — abre la URL de la licencia del conjunto de datos en una nueva pestaña (se muestra solo cuando el registro incluye un enlace de licencia)
* *Seleccionar* / *Deseleccionar* — agrega o elimina el tema MQTT del conjunto de datos en el panel lateral de suscripción

NOTE: Si un conjunto de datos tiene licencia, al hacer clic en *Seleccionar* se muestra un diálogo de confirmación de licencia. Debe ver y aceptar la licencia antes de que el tema se añada al panel lateral de suscripción.
...
|Cola
|Cola de prioridad para el procesamiento de descargas: `high_priority`, `small_files` (predeterminado) o `large_files`. Use `large_files` para temas masivos o de archivos grandes para evitar bloquear descargas menores.
|===
...
=== Credenciales (Suscripción manual)

La vista *Suscripción manual* (kbd:[Alt+4]) incluye una sección de autenticación para fuentes que requieren credenciales. Seleccione el tipo de autenticación con el botón de radio:
...
|Sin autenticación — para fuentes de datos públicas
...
NOTE: Las credenciales se almacenan en Redis y se envían con cada solicitud de descarga de la suscripción. La contraseña y el token se ocultan en las respuestas de la API.

=== Gestión de suscripciones

La vista Suscripciones lista todas las suscripciones activas con su tema, ruta de guardado y resumen del filtro. Hay dos acciones disponibles en cada entrada:

* *Editar* — abre un diálogo para modificar el directorio de guardado, el filtro, la cola y las credenciales de la suscripción. El tema no puede cambiarse; para suscribirse a un tema diferente debe crear una nueva suscripción.
* *Cancelar suscripción* — elimina la suscripción. Si era la última suscripción para ese tema, la conexión MQTT también se cierra.

Use *Recargar suscripciones* para actualizar la lista tras cualquier cambio realizado mediante la API.
...
|`queue`
|No
|Cola de prioridad para el procesamiento: `high_priority`, `small_files` (predeterminado) o `large_files`

|`credentials`
|No
|Autenticación para la fuente de datos (ver <<Credenciales-API>>)
|===
...
Actualice el directorio de guardado, el filtro, la cola o las credenciales de una suscripción sin recrearla:
...
Solo cambian los campos incluidos; los campos omitidos conservan sus valores actuales. `credentials` puede establecerse en `null` para eliminar la autenticación.
...
Las suscripciones que descargan desde fuentes protegidas por contraseña o token aceptan un objeto `credentials` opcional.
...
NOTE: La API oculta los secretos en las respuestas — `password` se omite y `token` se reemplaza por `***`. Las credenciales completas se almacenan en Redis y se usan solo durante la descarga.
