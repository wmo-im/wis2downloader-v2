...
The navigation drawer on the left (right in Arabic RTL mode) gives access to seven views. Each can also be reached with a keyboard shortcut:
...
|kbd:[Alt+4]
|*Manual Subscribe*
|Enter a topic, save directory, filter, queue, and credentials directly without browsing the catalogue

|kbd:[Alt+5]
|*Manage Subscriptions*
|List all active subscriptions, edit their settings, and unsubscribe from topics
...
|kbd:[Alt+7]
|*Documentation*
|Shows the project documentation
|===
...
* *View License* — opens the dataset's license URL in a new tab (only shown when the record includes a license link)
* *Select* / *Unselect* — adds or removes the dataset's MQTT topic to the subscription sidebar

NOTE: If a dataset carries a license, clicking *Select* shows a license confirmation dialog. You must view and accept the license before the topic is added to the subscription sidebar.
...
|Queue
|Priority queue for download processing: `high_priority`, `small_files` (default), or `large_files`. Use `large_files` for bulk or large-file topics to avoid blocking smaller downloads.
|===
...
=== Credentials (Manual Subscribe)

The *Manual Subscribe* view (kbd:[Alt+4]) includes an authentication section for data sources that require credentials.  Select the authentication type with the radio button:
...
|None (default)
|No authentication — used for public data sources
...
NOTE: Credentials are stored in Redis and sent with each download request for the subscription. The password and token are redacted in API responses.

=== Managing Subscriptions

The Subscriptions view lists all active subscriptions with their topic, save path, and filter summary. Two actions are available on each entry:

* *Edit* — opens a dialog to modify the subscription's save directory, filter, queue, and credentials. The topic cannot be changed; to subscribe to a different topic you must create a new subscription.
* *Unsubscribe* — removes the subscription. If it was the last subscription for that topic, the MQTT connection is also closed.

Use *Reload Subscriptions* to refresh the list after any changes made via the API.
...
|`queue`
|No
|Priority queue for processing: `high_priority`, `small_files` (default), or `large_files`

|`credentials`
|No
|Authentication for the data source (see <<Credentials>>)
|===
...
Update a subscription's save directory, filter, queue, or credentials without recreating it:
...
Only the fields you include are changed; omitted fields keep their current values. `credentials` can be set to `null` to remove authentication.
...
Subscriptions that download from password-protected or token-protected sources accept an optional `credentials` object.
...
NOTE: The API redacts secrets in responses — `password` is omitted and `token` is replaced with `***`. The full credentials are stored in Redis and used only during download.
