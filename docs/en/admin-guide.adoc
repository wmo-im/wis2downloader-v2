...
|User ID (UID)
|`10001` (default — configurable via `WIS2DOWNLOADER_UID` in `.env`)

|Group name
|`wis2`

|Group ID (GID)
|`988` (default — configurable via `WIS2DOWNLOADER_GID` in `.env`)
|===

The Celery worker writes files to `/data` inside the container, which maps to `HOST_DATA_PATH` on the host (default: `./downloads/`).

IMPORTANT: If you change `WIS2DOWNLOADER_UID` or `WIS2DOWNLOADER_GID` in `.env`, rebuild the images with `docker compose build` before restarting, and update the host user/group commands below to match.
...
|integer
|`5001`
|API bind port (internal container port)

|`WIS2DOWNLOADER_UID`
|integer
|`10001`
|User ID used when building container images. Must match the host directory owner for volume writes.

|`WIS2DOWNLOADER_GID`
|integer
|`988`
|Group ID used when building container images. Must match the host directory group for volume writes.

|`HOST_DATA_PATH`
|path
|`./downloads`
|Host path mounted as the download volume
...
|string
|`http://localhost`
|Public base URL of the deployment (server IP or hostname). Drives the defaults for the two variables below. Set this to your server's address when deploying remotely.

|`WIS2DOWNLOADER_SUBSCRIPTION_MANAGER_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:5002`
|Browser-facing URL of the subscription manager API

|`WIS2DOWNLOADER_GRAFANA_URL`
|string
|`${WIS2DOWNLOADER_BASE_URL}:3000`
|Browser-facing URL used to embed Grafana panels in the Dashboard view
...
The stack runs three Celery worker pools, each targeting a dedicated queue:

[cols="2,1,1,3"]
|===
|Service |Queue |Default Concurrency |Purpose
...
Adjust concurrency by editing the `--concurrency` flag in the relevant service's `command` in `docker-compose.yaml`:
...
When creating a subscription via the API, use the `queue` field to route downloads to a specific pool (`high_priority`, `small_files`, or `large_files`). The default is `small_files`.
...
# Specific service (use the worker queue name)
